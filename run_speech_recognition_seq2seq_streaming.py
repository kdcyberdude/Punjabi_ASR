...
import datasets
import torch
from datasets import DatasetDict, IterableDatasetDict, interleave_datasets, load_dataset, load_from_disk, concatenate_datasets, Audio
...
def load_custom_dataset_for_punjabi():
    # ds1 = load_dataset(creds.yt_ds_hub, cache_dir='./asr_ds_yt_cache')
    ds1 = load_from_disk('/mnt/pi/datasets/speech/yt_dataset')
    filtered_ds1_indexes_file = './cache_dir/alignment_result/selected_indexes/selected_indexes_yt_dataset_58.53_percent.json'
    ds1_selected_indexes = []
    with open(filtered_ds1_indexes_file) as f:
        ds1_selected_indexes = json.load(f)
    with open('cache_dir/alignment_result/num_selected_indexes/selected_indexes_yt_dataset_numbers_samples.json') as f:
        ds1_selected_indexes.extend(json.load(f))
    ...
    ds2 = load_from_disk('/mnt/sea/speech/punjabi_asr_datasets/all_except_stt_yt')
    filtered_ds2_indexes_file = './cache_dir/alignment_result/selected_indexes/selected_indexes_punjabi_asr_dataset_v2_92.23_percent.json'
    with open(filtered_ds2_indexes_file) as f:
        ds2_selected_indexes = json.load(f)
    with open('cache_dir/alignment_result/num_selected_indexes/selected_indexes_all_V2_dataset_numbers_samples.json') as f:
        ds2_selected_indexes.extend(json.load(f))
    ...
    ds = DatasetDict({
        ...
        'eval': concatenate_datasets([ds1['test'], ds2['test']])
    })
...
def main():
    ...
    raw_datasets = load_custom_dataset_for_punjabi()
